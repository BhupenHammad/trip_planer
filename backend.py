...
def flight_agent(state: TravelState):
    ...
    try:
        departure_iata, arrival_iata = parse_route(query)
        airports = {
            "departure": AIRPORTS.get(departure_iata),
            "arrival": AIRPORTS.get(arrival_iata),
            "departure_iata": departure_iata,
            "arrival_iata": arrival_iata,
        }
        airlines = (
            "Live airline catalog unavailable on the current AviationStack plan. "
            "Use typical route knowledge and label airline information as estimated."
        )
...
def itinerary_agent(state: TravelState):
    # Limit the amount of tool output sent to the LLM
    def trim_text(value, max_chars=5000):
        if not value:
    ...
    flight_results = trim_text(
        state.get("flight_results", ""),
        max_chars=4000
    )

    hotel_results = trim_text(
        state.get("hotel_results", ""),
        max_chars=4000
    )

    weather_results = trim_text(
        state.get("weather_results", ""),
        max_chars=3000
    )

    budget_results = trim_text(
        state.get("budget_results", ""),
        max_chars=3000
    )

    trip_constraints = trim_text(
        state.get("trip_constraints", {}),
        max_chars=2000
    )

    user_query = trim_text(
...
- Consider available flights and hotels.
...
- Create a draft ready for human review.
"""
